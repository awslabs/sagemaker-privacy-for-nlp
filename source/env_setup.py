...
def env_setup_notebook_instance() -> None:
    ...
    # Required for annoy 1.17.0
    bash("sudo yum install -y gcc72 gcc72-c++")
    ...
    bash(f"""
    export PIP_DISABLE_PIP_VERSION_CHECK=1
    export CXX=/usr/bin/g++72
    export CC=/usr/bin/gcc72
    {py_exec} -m pip install --no-cache-dir -r {CURRENT_FOLDER}/sagemaker/requirements.txt
    {py_exec} -m pip install -e {CURRENT_FOLDER}/sagemaker/
    """)
...
def env_setup_studio() -> None:
    ...
    py_exec = get_executable()
    logging.info('Installing pip packages.')
    # --no-cache-dir is required because torch is a large download that causes the container to run out of memory
    bash(f"""
    export PIP_DISABLE_PIP_VERSION_CHECK=1
    {py_exec} -m pip install -e {CURRENT_FOLDER}/sagemaker/
    """)
